...
public class UpdateCalendarEventRequestValidator : AbstractValidator<UpdateCalendarEventRequest>
{
    public UpdateCalendarEventRequestValidator()
    {
        ...
        // EditScope is required when the event has a recurrence rule
        RuleFor(x => x.EditScope)
            .NotNull().WithMessage("EditScope is required when editing a recurring event")
            .When(x => !string.IsNullOrEmpty(x.RecurrenceRule));

        RuleFor(x => x.EditScope)
            .IsInEnum().WithMessage("Invalid edit scope")
            .When(x => x.EditScope.HasValue);

        // OccurrenceStartTimeUtc is required for ThisOccurrence and ThisAndFuture edits
        RuleFor(x => x.OccurrenceStartTimeUtc)
            .NotNull().WithMessage("OccurrenceStartTimeUtc is required for ThisOccurrence and ThisAndFuture edits")
            .When(x => x.EditScope is RecurrenceEditScope.ThisOccurrence or RecurrenceEditScope.ThisAndFuture);
    }
}
